...
export class GetCustomOrderController extends BaseController {
  ...
  protected async executeImpl(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    if (!hasAccess) {
      throw AppError.forbidden('You do not have permission to view this custom order');
    }

    const customOrder = await this.customOrderService.getCustomOrderById(id);

    if (!customOrder) {
      throw AppError.notFound('Custom order not found');
    }

    ApiResponse.success(res, customOrder, 'Custom order retrieved successfully');
  }
}
