  ...
  implements IProductRepository
{
  ...
  async createProduct(sellerId: string, data: CreateProductDto): Promise<ProductWithDetails> {
    try {
      // Chỉ thực hiện database operations
      const product = await this.prisma.$transaction(async (tx) => {
        ...
        // Liên kết với categories nếu có
      ...
      // Lấy thêm categories sau khi tạo
      const productWithCategories = await this.findByIdWithDetails(product.id);
      return productWithCategories as ProductWithDetails;
    } catch (error) {
      this.logger.error(`Database error in createProduct: ${error}`);
      // Chỉ xử lý database errors, không xử lý business errors
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        // Handle specific Prisma errors
        if (error.code === 'P2002') {
          throw new Error('Duplicate product information');
        }
      }
      throw error;
    }
